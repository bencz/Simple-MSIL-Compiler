...
#include <stdio.h>
/* DAL updates for interacting with the prototype MSIL compiler */
#define isatty _isatty
#define setmode _setmode
#define chmod _chmod
/* DAL end MSIL updates */
...
void undoReversibleTransformation_fast ( FILE* dst )
{
   ...
   cftab[0] = 0;
   for (i = 1; i <= 256; i++) { cftab[i] = unzftab[i-1]; /*printf("%d\n", unzftab[i-1]); */}
   for (i = 1; i <= 256; i++) cftab[i] += cftab[i-1];
...
{
   #if defined(BZ_UNIX) || defined(BZ_EMX)
   IntNative      retVal;
   struct MY_STAT statBuf;
